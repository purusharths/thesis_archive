...
#include "fischerKPP_GRAF_epi.h"

#ifdef WITH_GPERF
#include "profiler.h"
#endif

static const char *PARAM_FILENAME = "fischerKPP_GRAF_epi.xml";
...
public:
  FisherKolmogorovGrowth(const std::string &param_filename,
    ...
    if (rank_ == 0) {
      ...
      this->grf.reinitilize(SIZE_, 12, 500, 4000);
...
void FisherKolmogorovGrowth::EvalGrad(const VectorType &in, MatrixType *out) {
  ...
  LocalPoissonAssembler local_asm(vec);
