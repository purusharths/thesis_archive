...
def process_dataframes(dataframes):
    for df in dataframes:
        if 'value' in df.columns:
            df['value'] = np.clip(df['value'], 0, 1)  # Ensure 'value' is between 0 and 1
            df['value'] = np.where(df['value'] > 1.5, 0, df['value'])  # Set values greater than 1.5 to 0
    return dataframes
...
def plot_and_save(folder_name, dfs):
    ...
    plt.savefig(pdf_filename, dpi=100, bbox_inches="tight")
