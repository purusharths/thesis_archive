...
DataType MAG_GR = 10;
static std::string npyfilename = "<example>";

std::vector<std::vector<double>> vect;


int load_get_std_vector(const std::string &filename, std::vector<std::vector<double>> &vec) {
  xt::xarray<double> arr;
  std::cout << "++++++++++ LOADING VECTOR " << filename <<"+++++++++++" << std::endl;
  ...
  std::cout << "** Vector Loaded -> vec[10][20]: " << vec[10][20] << std::endl;
  return 0;
}

void pass_f_from_main(const std::string  & name){
  npyfilename = name;
  std::cout << npyfilename << std::endl;
  load_get_std_vector(npyfilename, vect);
}
// std::string npyfilename = "/home/purusharth/Documents/sandbox/para/matern2_1000.npy";



// std::string npyfilename = "/home/hd/hd_hd/hd_gs270/RFs/30k_lowalpha2.npy";
// std::string npyfilename = "/home/hd/hd_hd/hd_gs270/RFs/30k_highalph2.npy";


/*
  std::string matern_filename = "/home/purusharth/Documents/sandbox/para/matern2_1000.npy";
  std::vector<std::vector<double>> matern_vect;
  int throwaway2 = load_get_std_vector(matern_filename, matern_vect);
*/
...
public:
  ...
  void operator()(const Element<DataType, DIM> &element,
    ...
    for (int q = 0; q < num_q; ++q) { // loop over quadrature points
      ...
      //      (x(q)[1]*SCALE_), 1);
      double D;
      D = vect[std::round(x(q)[0] * SCALE_)][std::round(x(q)[1] * SCALE_)];
      /*if (time_ < 15){
                  D = matern_vect[std::round(x(q)[0] * SCALE_)][std::round(x(q)[1] * SCALE_)];
       }else{
        // LOG_INFO("LOLLLL", "matern");
               D = vect[std::round(x(q)[0] * SCALE_)][std::round(x(q)[1] * SCALE_)];
        }*/
      // if (time_ > 0.85){
      ...
      for (int i = 0; i < num_dof; ++i) { // loop over test DOFs <-> test function v
        for (int j = 0; j < num_dof; ++j) { // loop over trial DOFs <-> trial function u
          // std::cout << dt_ << " ";
          if (mat_num_ == 1) {
            carrying_capacity = carrying_capacity_1_ * D;
            diffusion_coeff = diffusion_1_ * D;
          }
          if (mat_num_ == 2) {
          ...
          if (mat_num_ == 3) {
            ...
            // }
            carrying_capacity = growth_rate_3_ * D;
