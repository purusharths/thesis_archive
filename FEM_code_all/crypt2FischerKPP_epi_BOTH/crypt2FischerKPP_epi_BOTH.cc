...
#include "crypt2FischerKPP_epi_BOTH.h"

#ifdef WITH_GPERF
#include "profiler.h"
#endif

static const char *PARAM_FILENAME = "crypt2FischerKPP_epi_BOTH.xml";
...
public:
  ...
  void run() {
    ...
    // this->grf.compute();
    prefix_path_ = params_["Mesh"]["OutputPath"].get<std::string>("./");
    build_initial_mesh();
    ...
    while (t_ < t_end) {
      ...
      if (ts_ % visualization_frequency_ == 0.) {
        timer.reset();
        timer.start();
        LOG_INFO("do", "Visualize Solution "); // Visualize the solution and the errors.
        visualize();
        timer.stop();
        LOG_INFO("duration", timer.get_duration());
      } else {
...
private:
  ...
  std::vector<int>  dirichlet_dofs_; // Dof values for Dirichlet boundary conditions.
  ...
  DataType mean_, variance_, normed_, ic_x_location_, ic_y_location_;
  DataType theta_;
...
void FisherKolmogorovGrowth::initial_condition() {
  ...
  local_asm.set_parameters(ic_x_location_, ic_y_location_, variance_, normed_);
...
void FisherKolmogorovGrowth::prepare_system() {
  ...
  ic_x_location_ = params_["InitialCondition"]["x_location"].get<DataType>();
  ic_y_location_ = params_["InitialCondition"]["y_location"].get<DataType>();  
...
void FisherKolmogorovGrowth::EvalGrad(const VectorType &in, MatrixType *out) {
  ...
  local_asm.set_parameters(
      ...
      params_["Parameters"]["diffusion_3"].get<DataType>(),
      params_["Parameters"]["growth_rate_3"].get<DataType>(),
      t_, a_, b_, theta_, dt_);
...
void FisherKolmogorovGrowth::visualize() {
  ...
  std::string final_filename = prefix_path_ + name.str();
  VTKWriter<DataType, DIM> vtk_writer(visu, this->comm_, MASTER_RANK);
  vtk_writer.write(final_filename);
}
